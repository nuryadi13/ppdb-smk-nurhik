...
if (isset($_POST['submit'])) {
    ...
    if (empty($nama_siswa) || empty($nama_orang_tua) || empty($kelas)) {
    ...
    if (
        ...
        if (
            ...
            $sql = "INSERT INTO siswa (nama_siswa, kelas, nama_orangtua, fotosiswa, ktp, kk) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($koneksi, $sql);

            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssss", $nama_siswa, $kelas, $nama_orang_tua, $fotosiswa_path, $ktp_path, $kartu_keluarga_path);
